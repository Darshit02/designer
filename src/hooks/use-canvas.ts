...
export const useInfiniteCanvas = () => {
  ...
  const isPointInShape = (point: Point, shape: Shape): boolean => {
    switch (shape.type) {
      ...
      case "text":
        ...
        const textHight = shape.fontSize * 1.2;
        const padding = 8;
        return (
          point.x >= shape.x - 2 &&
          point.x <= shape.x + textWidth + padding + 2 &&
          point.y >= shape.y - 2 &&
          point.y <= shape.y + textHight + padding + 2
        );
      default:
        return false;
    }
  ...
  const onPointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (!isButton) {
      ...
      console.log("Not preventing Default", target);
      return;
    }
    ...
    if (touchMapRef.current.size <= 1) {
      ...
      if (e.button === 0) {
        if (currentTool == "select") {
          ...
          if (hitShape) {
            const isAlredyselected = selectedShapes[hitShape.id];
            if (!isAlredyselected) {
      ...
      Object.keys(initialShapePositionRef.current).forEach((id) => {
        ...
        if (shape && initialPos) {
          if (
            ...
            const initialPoint = initialPos.points;
            if (initialPoint) {
              const newPoints = initialPoint.map((point) => ({
                x: point.x + deltaX,
                y: point.y + deltaY,
              }));
              dispatch(
  ...
  const finalizeDrawingIfAny = (): void => {
    ...
    if (draft) {
      ...
      if (w > 1 && h > 1) {
        if (draft.type === "frame") {
          console.log("Adding The Frame", { x, y, w, h });
  ...
  const onPointerCancle: React.PointerEventHandler<HTMLDivElement> = (e) => {
    onPointerUp(e);
  };
  ...
  useEffect(() => {
    ...
    const handleResizeMove = (e: CustomEvent) => {
      ...
      const loacalY = clientY - rect.top;
      ...
      if (
        ...
        const xs = shape.point.map((p: { x: number; y: number }) => p.x);
        const ys = shape.point.map((p: { x: number; y: number }) => p.y);
    ...
    window.addEventListener(
      "shape-resize-end",
      handelResizeEnd as EventListener
    );
